...
## Match Output: "OnCore_Matched_CNV.tsv"
...
if (isTRUE(Internal_match)) {
  ...
  DF_Output_CNV_OnCore <- data.frame(matrix(NA, ncol = (ncol_CNV + ncol_OnCore)))
  colnames(DF_Output_CNV_OnCore) <- append(CNV.colnames, colnames(OnCore_Biomarker_Report))
  ...
  for (patient_num in 1:length(patient.list)) {
    ...
    if (nrow(DF_patient) > 0) {
      ...
      DF_patient$OnCore_CNV_Status <- NA
                  ...
                  if (isTRUE(disease.cat_id %in% disease_category_name_patient)) {
                    ...
                    for (site_num in 1:length(Disease.Site.patient)) {
                      ...
                      if (isTRUE(disease.site_id %in% disease_site_name_patient)) {
                        ...
                        DF_patient$OnCore_CNV_Status[pt_rowNo] <- 
                        ...
                        DF_Output_CNV_OnCore <- rbind(DF_Output_CNV_OnCore, DF_Output_pre)
                        
                        remove(DF_Output_pre,Trial_INFO,OnCore_No,patient)
                        
                      } else {
                        DF_patient$OnCore_CNV_Status[pt_rowNo] <- "Disease Site criteria NOT satisfied"
                      }
