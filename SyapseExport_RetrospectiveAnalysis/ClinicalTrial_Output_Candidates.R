...
if (isTRUE(Internal_match)) {
  ...
  remove(Output_SNVIndel_OnCore,continue_SNVIndel,colnames.output,colnames.OnCore,colnames.SNVIndel.original)
...
if (isTRUE(NCI_match)) {
  ...
  remove(Output_SNVIndel_Variant,Output_SNVIndel_NonHotspot,continue_SNVIndel_Variant,continue_SNVIndel_NonHotspot,
         colnames.NCI.Variant,colnames.NCI.NonHotspot,colnames.SNVIndel.original,colnames.output,colnames.NCI.output)
...
if (isTRUE(length(patient.list.matched) > 0)) {
  ...
  for (patient_num in 1:length(patient.list.matched)) {
    ...
    #----------------------------------------------
    if (length(patient.oncore.matched) > 0) {Output_OnCore_FINAL_Patient <- Output_OnCore_FINAL[which(Output_OnCore_FINAL$PatientID == patient_id), ]}
    if (length(patient.nci.matched) > 0) {Output_NCI_FINAL_Patient <- Output_NCI_FINAL[which(Output_NCI_FINAL$PatientID == patient_id), ]}
    ...
    #----------------------------------------------
    if (isTRUE(Internal_match)) {
      if (nrow(Output_OnCore_FINAL_Patient) > 0 ) {
        ...
        remove(Output_OnCore_FINAL_Patient,OnCoreNo.list)
    ...
    #----------------------------------------------
    if (isTRUE(NCI_match)) {
      if (nrow(Output_NCI_FINAL_Patient) > 0) {
        ...
        remove(Output_NCI_FINAL_Patient,NCIArm.Variant.list,num_variant)
      }
    }
