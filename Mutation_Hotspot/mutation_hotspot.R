...
Lollipop_Plot <- function(variant_type, assay,
  ...
  # Specify y-axis parameters = yaxis_max, y_tick, y_legend
  if (loc.freq.max <= 10) {
    yaxis_max = 10
    y_tick = 1
    y_legend = -1
  ...
  if (isTRUE(static_plot)) {
    ...
    if (variant_type == "All") {
      point_color = "black"
    } else if (variant_type == "Pathogenic") {
      point_color = "firebrick4"
    } else if (variant_type == "Unknown significance") {
      point_color = "darkslategrey"
    } else if (variant_type == "Benign") {
  ...
  if (isTRUE(dynamic_plot)) {
    ...
    plot_dynamic <- 
      ...
      labs(title = paste(unique(DF_gene_INFO$symbol), " variants from ", 
                         assay, " (n=",
                         sum(data_var_FULL$var.freq[data_var_FULL$pathogenicity.status == "All"]),
                         " total)", sep="")) +
    ...
    assign("plot_dynamic", plot_dynamic, envir = .GlobalEnv)
  }
}
  ...
  for (gene_num in 1:length(gene.list)) {
    ...
    if (length(allele.count) > 0) {
      data_var.patho = data.frame(var.pos = as.numeric(names(allele.count)), 
                                  var.freq = as.vector(allele.count),
                                  pathogenicity.status = "Pathogenic")
    }
    ...
    if (length(allele.count) > 0) {
      data_var.vus = data.frame(var.pos = as.numeric(names(allele.count)), 
                                var.freq = as.vector(allele.count),
                                pathogenicity.status = "Unknown significance")
    }
    ...
    if (as.numeric(table(DF$base.gene == gene_id)["TRUE"]) > 0) {
      Lollipop_Plot(variant_type = "All", assay = assay,
                    dynamic_plot = TRUE)
      pNum = 1
      pList[[pNum]] <- plot_static
      
      if ("Pathogenic" %in% index.sig | "Likely Pathogenic" %in% index.sig) {
        Lollipop_Plot(variant_type = "Pathogenic", assay = assay)
        pNum = pNum + 1
        pList[[pNum]] <- plot_static
      }

      if ("Unknown significance" %in% index.sig | "Unknown" %in% index.sig) {
        Lollipop_Plot(variant_type = "Unknown significance", assay = assay)
        pNum = pNum + 1
        pList[[pNum]] <- plot_static
      }
      
      if ("Likely Benign" %in% index.sig) {
        Lollipop_Plot(variant_type = "Benign", assay = assay)
        pNum = pNum + 1
        pList[[pNum]] <- plot_static
      }
      ...
      # Output dynamic plot to
      #----------------------------------------------
      p <- ggplotly(plot_dynamic)

      file_id_online = paste(gene_id, "_", assayName, sep="")
      api_create(p, filename = file_id_online,
                 fileopt = "overwrite", 
                 sharing = "public"     # "private" option only avaiable for paid subscriptions
                 )
    }
  }
}
...
STAMPv2_Annotation <- 
  import_list(paste(STAMP_annotation_timestamp, "_STAMPv2_Annotation.xlsx", sep=""), setclass = "tbl")
...
remove(plot_dynamic,plot_static,data_var_FULL,DF_gene_INFO)
