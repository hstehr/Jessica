...
## PIPELINE
#----------------------------------------------
Mutation_Pipeline <- function (gene.list, DF, assay, outdir) {
  ...
  for (gene_num in 1:length(gene.list)) {
    ...
    if (as.numeric(table(DF$VariantGene == gene_id)["TRUE"]) > 0) {
      ...
      if (isTRUE(saveStaticPlots)) {
        ...
        gpanels <- annotate_figure(gpanels, 
                                   top = text_grob(paste(gene_id, " variants from ", assay, 
                                                   ...
                                                   face = "bold", size = 18))
        file_id = paste(outdir, gene_id, "_", assay, ".jpg", sep="")
      ...
      if (isTRUE(saveDynamicPlots)) {
        ...
        file_id_online = paste(gene_id, "_", assay, sep="")
        filename_Full = paste("STAMPEDE/Gene_Lollipop/", file_id_online, sep="")
...
STAMPv2_Annotation <- 
  suppressMessages(import_list("/Users/jessicachen/Documents/ClinicalDataScience_Fellowship/STAMP/2016-08-23_STAMPv2_Annotation.xlsx", 
                               setclass = "tbl"))
...
## Subset STAMP DF, if applicable 
#----------------------------------------------
DF <- STAMP_DF[STAMP_DF$AssayName == "STAMP - Solid Tumor Actionable Mutation Panel (130 genes)", ]

cat(paste("The STAMP entries to be analyzed compromise of ", nrow(DF), " entries, ", length(unique(DF$VariantGene)), 
          " genes and ", length(unique(DF$PatientID)), " patients.", sep=""), "\n")

# Structure columns 
col.change <- c("AssayOrderingPhysician", "PatientID", "PatientGender", 
                "AssayName", "VariantCHR", "PrimaryTumorSite")
...
#----------------------------------------------
Mutation_Pipeline(gene.list = sort(unique(DF$VariantGene)),
                  DF = DF,
                  assay = "STAMP_v2",
                  outdir = outdir)

remove(Changes,Tiles,STAMPv2_Annotation, col.change, gene.DF, domain.DF,Temp_DF,gene.list,DF,
       Lollipop_Plot,Mutation_Pipeline,plot_dynamic_int,plot_static,data_var_FULL,DF_gene_INFO)
